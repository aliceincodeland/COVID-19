...
def prepare_data_for_plot(topic, country, moving_avg_window):
    ...
    for i in range(1, len(arr)):
        #print(arr[i][1])
        try:
            if arr[i][1].casefold() in country.casefold(): #country.casefold():
...
moving_avg_window = 15  # int(input('Enter the number of days over which the moving average is to be calculated: '))

dates, moving_avg_confirmed, month_beginning_indices, total_confirms = prepare_data_for_plot('c',
                                                                                             country, moving_avg_window)
...
my_plot.set_xlabel('Total Deaths: ' + str(total_deaths) + ',  Total Confirmed Cases: ' + str(total_confirms)  + ',   Ratio = ' + str(total_deaths/total_confirms))
